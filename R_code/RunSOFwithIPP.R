# Iterative Partial Push with SOF
# (c) Ralph Foorthuis
# 2020

SOFwithIPP <- function(datset, QuantileDenominator = 100, QuantileFilterBoost = -9999, SampleSize=20, Normalize=TRUE, TestMode=TRUE) {
  ...
  # Run SOF
  cat("Starting mixed data analysis with SOF.\n")
  SOFanomalies <- qsp(datsetMM, sample.size = SampleSize, normalize = Normalize)
  HDAnomalyTableSOF <- data.frame(Ano_IDM=seq(1:length(SOFanomalies)), AveAnoScoreMixed=SOFanomalies)
  
  # Conduct numerical analysis on the continuous variables
  cat("Starting numerical analysis with SOF.\n")
  SOFanomalies <- qsp(as.matrix(datset[,VarsTable[VarsTable$DataType=="numerical", "VariableName"]]), sample.size = SampleSize, normalize = Normalize)
  HDAnomalyTableSOF$Ano_IDN <- seq(1:length(SOFanomalies))
  HDAnomalyTableSOF$AveAnoScoreNum <- SOFanomalies
  
  # Reverse scores because higher scores are more anomalous with SOF (as opposed to what IPP expects)
  HDAnomalyTableSOF$AveAnoScoreMixed <- 1 + max(HDAnomalyTableSOF$AveAnoScoreMixed, na.rm=TRUE) - HDAnomalyTableSOF$AveAnoScoreMixed
  HDAnomalyTableSOF$AveAnoScoreNum <- 1 + max(HDAnomalyTableSOF$AveAnoScoreNum, na.rm=TRUE) - HDAnomalyTableSOF$AveAnoScoreNum
  ...
  while (StartIteration==TRUE) {
    ...
    IDsOfNAs <- which(is.na(HDAnomalyTableSOF$HDAscore)) # Identify rows with empty Score cells. In the first iteration this will be all rows, and this set get be smaller after each iteration because more and more rows will have been scored.
    ...
    HDtemp <- HDAnomalyTableSOF[Ano_IDs_ToUpdate,] # Create data.frame with cases to update
  ...
  cat(paste("\n\nNumber of potential HDA cases identified:", nrow(datset)-length(which(is.na(HDAnomalyTableSOF$HDAscore))),  "\n"))
  ...
  HDAnomalyTableSOF$HDAscore <- ifelse(is.na(HDAnomalyTableSOF$HDAscore), (1 + max(HDAnomalyTableSOF$AveAnoScoreNum, na.rm=TRUE) - HDAnomalyTableSOF$AveAnoScoreNum) + QuantileDenominator, HDAnomalyTableSOF$HDAscore) # If the Iteration has a value then keep that. Otherwise (if it's empty), then fill it with the anomaly score of numerical/density-based data anomalies (and add the QuantileDenominator (i.e. the highest iteration) to ensure they are higher than the identified HDA anomalies).  
  
  # Return HDA scores
  colnames(HDAnomalyTableSOF)[1] <- "Ano_ID"
  return(HDAnomalyTableSOF[, c("Ano_ID", "HDAscore")]) 
}

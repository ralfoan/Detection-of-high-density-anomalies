# Iterative Partial Push with KNN_AGG
# (c) Ralph Foorthuis
# 2020

KNN_AGGwithIPP <- function(datset, QuantileDenominator = 100, QuantileFilterBoost = -9999, k_min=10, k_max=15, Normalize=TRUE, TestMode=TRUE) {
  ...
  VarsTable <- data.frame(VariableName=character(), DataType=character(), stringsAsFactors=FALSE)  # The set of variables and their datatypes being analyzed. 
  ...
  # Reverse scores because higher scores are more anomalous with KNN_AGG (as opposed to what IPP expects)
  HDAnomalyTableKNN_AGG$AveAnoScoreMixed <- 1 + max(HDAnomalyTableKNN_AGG$AveAnoScoreMixed, na.rm=TRUE) - HDAnomalyTableKNN_AGG$AveAnoScoreMixed
  HDAnomalyTableKNN_AGG$AveAnoScoreNum <- 1 + max(HDAnomalyTableKNN_AGG$AveAnoScoreNum, na.rm=TRUE) - HDAnomalyTableKNN_AGG$AveAnoScoreNum
  ...
  while (StartIteration==TRUE) {
    ...
    IDsOfNAs <- which(is.na(HDAnomalyTableKNN_AGG$HDAscore)) # Identify rows with empty Score cells. In the first iteration this will be all rows, and this set get be smaller after each iteration because more and more rows will have been scored.
    ...
    HDtemp <- HDAnomalyTableKNN_AGG[Ano_IDs_ToUpdate,] # Create data.frame with cases to update
  ...
  cat(paste("\n\nNumber of potential HDA cases identified:", nrow(datset)-length(which(is.na(HDAnomalyTableKNN_AGG$HDAscore))),  "\n"))
  ...
  HDAnomalyTableKNN_AGG$HDAscore <- ifelse(is.na(HDAnomalyTableKNN_AGG$HDAscore), (1 + max(HDAnomalyTableKNN_AGG$AveAnoScoreNum, na.rm=TRUE) - HDAnomalyTableKNN_AGG$AveAnoScoreNum) + QuantileDenominator, HDAnomalyTableKNN_AGG$HDAscore) # If the Iteration has a value then keep that. Otherwise (if it's empty), then fill it with the anomaly score of numerical/density-based data anomalies (and add the QuantileDenominator (i.e. the highest iteration) to ensure they are higher than the identified HDA anomalies).  
